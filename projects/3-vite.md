...
As we discovered in the [ESModules lecture](https://github.com/The-Marcy-Lab-School/marcy-curriculum-docs/blob/main/mod-3-html-css/esmodules.md), we can't use ESModules with the normal `file://` protocol without running into CORS (cross-origin resource sharing) issues. We need to use the `http://` protocol to enable our files to import values from each other.
