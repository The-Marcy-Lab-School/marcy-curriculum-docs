...
* [Essential Questions](5-schema-design-and-normalization.md#essential-questions)
* [Key Concepts](5-schema-design-and-normalization.md#key-concepts)
* [What is Schema Design?](5-schema-design-and-normalization.md#what-is-schema-design)
* [Normalization](5-schema-design-and-normalization.md#normalization)
  * [Normalization Rule #1: Unique Primary Keys](5-schema-design-and-normalization.md#normalization-rule-1-unique-primary-keys)
  * [Normalization Rule #2: Atomic Values](5-schema-design-and-normalization.md#normalization-rule-2-atomic-values)
    * [Don't Forget The Primary Key!](5-schema-design-and-normalization.md#dont-forget-the-primary-key)
  * [Normalization Rule #3: Primary Key Dependency](5-schema-design-and-normalization.md#normalization-rule-3-primary-key-dependency)
    * [Association Tables for Many-To-Many Relationships](5-schema-design-and-normalization.md#association-tables-for-many-to-many-relationships)
* [Designing a Multi-Table Schema](5-schema-design-and-normalization.md#designing-a-multi-table-schema)
  * [Create Independent Tables First](5-schema-design-and-normalization.md#create-independent-tables-first)
  * [Use `REFERENCES` for Foreign Keys](5-schema-design-and-normalization.md#use-references-for-foreign-keys)
  * [Creation order matters](5-schema-design-and-normalization.md#creation-order-matters)
  * [`UNIQUE` Constraint on Multiple Columns](5-schema-design-and-normalization.md#unique-constraint-on-multiple-columns)
  * [Complete Schema](5-schema-design-and-normalization.md#complete-schema)
  * [Schema Investigation](5-schema-design-and-normalization.md#schema-investigation)
  * [See It in Action](5-schema-design-and-normalization.md#see-it-in-action)
* [Practice](5-schema-design-and-normalization.md#practice)
* [ERDs: Design Before You Code (Extension)](5-schema-design-and-normalization.md#erds-design-before-you-code-extension)
* [Normalization Rules Challenge](5-schema-design-and-normalization.md#normalization-rules-challenge)
...
* **ERD (Entity Relationship Diagram)** — a visual diagram showing each table's columns and the relationships between tables. _(See the Extension section.)_
...
Understanding the principles of normalization will allow you to defend and explain the _why_ behind every decision you make in your schema design.
...
![A table of orders with order\_id, customer\_id, customer\_name, customer\_address, products\_purchased, and product\_prices.)](../.gitbook/assets/normalization-1.png)
...
<summary><strong>Q: Where do you see corrupted (inaccurate and/or inconsistent) data? Where do you see redundant data?</strong></summary>
...
<summary><strong>Q: What is the primary key in the <code>orders</code> table? How do you know?</strong></summary>
...
Consider this `orders` table again and notice that both `products_purchased` and `product_prices` are _not_ atomic.

![A table of orders with order\_id, customer\_id, customer\_name, customer\_address, products\_purchased, and product\_prices.](../.gitbook/assets/normalization-1.png)

<details>

<summary><strong>Q: What difficulties could storing multiple values in one cell create?</strong></summary>
...
![The same orders table where products\_purchased and product\_prices each have one value per row. Every cell is now atomic.](../.gitbook/assets/normalization-2.png)

We've satisfied the rule of atomicity! But we've caused another issue.

<details>

<summary><strong>Q: This solution introduces a new issue, can you see it?</strong></summary>
...
Remember, every table needs a single unique, primary key to identify each row. We _could_ do this by adding a column called `order_product_id`:

{% hint style="danger" %}
<img src="../.gitbook/assets/normalization-3-bad.png" alt="This breaks the third rule of normalization: every column must depend solely on the primary key." data-size="original">
...
<summary><strong>Q: Which columns are dependent on a non-primary key column? Look for rows that have pairs of columns whose values are always the same.</strong></summary>
...
![The single "flat" table has been split into four tables](../.gitbook/assets/normalization-3-good.png)
...
![The order\_products table sits between orders and products, connecting them.](../.gitbook/assets/normalization-one-to-many.png)
...
* The _combination_ of those two foreign keys must be unique (e.g. order `5` shouldn't have product `3` listed twice)
* It can carry additional columns that describe the relationship, like `product_quantity`

<details>

<summary><strong>Q: Why can't we just add the product information directly to the <code>orders</code> table?</strong></summary>
...
<summary><strong>Q: What tables do we need? What are their relationships?</strong></summary>
...
<summary><strong>What columns does each table need?</strong></summary>
...
`enrollments` links a student to a class. A student should only be enrolled in the same class once — but `student_id` and `class_id` individually can (and should) repeat. The constraint we need is on the _combination_:
...
<summary><strong>Q: Why does <code>classes</code> get the <code>teacher_id</code> and not the <code>teacher_name</code>?</strong></summary>
...
<summary><strong>Q: Why does the <code>enrollments</code> table exist? What would go wrong if you tried to store enrollment data directly on the <code>students</code> table instead?</strong></summary>
...
<summary><strong>Q: What happens if you try to <code>CREATE TABLE classes</code> before <code>CREATE TABLE teachers</code>?</strong></summary>
...
<summary><strong>Q: What does Postgres do if you try to INSERT a row into <code>classes</code> with a <code>teacher_id</code> that doesn't exist in <code>teachers</code>?</strong></summary>
...
<summary><strong>Q: Why does <code>enrollments</code> need <code>UNIQUE (student_id, class_id)</code> rather than just making <code>student_id</code> or <code>class_id</code> individually unique?</strong></summary>
...
The constraint we actually want is on the _pair_: a student can be in many classes, and a class can have many students — but the same student cannot be in the same class twice. `UNIQUE (student_id, class_id)` expresses exactly that rule.
...
![An ERD maps contents of database tables (entities) and the relationships between them.](../.gitbook/assets/authors-books-erd.png)

Creating a diagram like this gives everyone a single source of truth _before_ anyone writes a line of application code.
...
<summary><strong>Q: What is the point of creating a diagram like this vs. using SQL to create the database directly in PostgreSQL?</strong></summary>
...
| employee\_id | employee\_name | department\_id | department\_name | project\_names              |
| ------------ | -------------- | -------------- | ---------------- | --------------------------- |
...
<summary><strong>Which rules does this table break?</strong></summary>
...
<summary><strong>How would you fix it?</strong></summary>
...
| employee\_project\_id | employee\_id | project\_id |
| --------------------- | ------------ | ----------- |
| 1                     | 1            | 1           |
| 2                     | 1            | 2           |
| 3                     | 2            | 2           |
